    ...
    <div className="App">
        ...
        <div className="menu-overlay">
          ...
          <div className="skin-options">
            {SkinImgs.map((skin, index) => (
              <img key={index} src={skin} alt={`Skin ${index + 1}`} onClick={() => setCurrentSkinIndex(index)} style={{ width: '80px', height: '80px' }} />
            ))}
          </div>
        </div>
      )}
      ...
      {showDiceSettings && (
        <div className="menu-overlay right">
          ...
          <h2>Flip Coin</h2>
          <button className="adjust-btn" onClick={flipCoin}>Flip Coin</button>
          {coinImages.length > 0 && (
